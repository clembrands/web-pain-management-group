...
const filters = [...hub.matchAll(/data-filter="\.([a-z-]+)"/g)].map((m) => m[1]);
const grid = [...hub.matchAll(/grid-item ([^"]*?) witr_all_mb_30"[\s\S]*?src="([^"]+)"[\s\S]*?<a href="([^"]*)"/g)].map(
  ([, cls, logo, link]) => ({ state: cls.split(/\s+/).find((c) => c in STATE_NAMES), logo, link: decode(link) }),
);
...
for (const p of live.filter((p) => p.path.startsWith("/em_portfolios/"))) {
  ...
  const block = bodyText(page)
    ...
    .replace(/ More Info$/, "")
    .trim();
  const images = new Set(page.match(/wp-content\/uploads\/[^"\s,]+?\.(?:png|jpe?g|webp|gif)/gi) ?? []);
  portfolios.push({ slug, name: p.title.replace(/ – Pain Management Group$/, ""), block, images });
}

const partners = grid.map((g) => {
  const file = g.logo.split("/").pop().replace(/\.\w+$/, "");
  const match = portfolios.find((p) => [...p.images].some((i) => i.includes(file)));
  if (!match) throw new Error(`No /em_portfolios/ page uses logo ${g.logo}`);
  const addr = match.block.match(/([A-Za-z .'-]+?),? ([A-Z]{2})\.? (\d{5})/);
  const phone = match.block.match(/P: ?([\d().\- ]{10,14}\d)/)?.[1].trim() ?? "";
  let city = addr?.[1].split(/\b(?:Dr|Drive|St|Street|Ave|Avenue|Rd|Road|Blvd|Hwy|Way|Ste|Suite \w+)\.?\s/).pop().trim() ?? "";
  let abbr = addr?.[2] ?? "";
  let source = "live site";
  if (CITY_FIX[match.slug]) city = CITY_FIX[match.slug];
  if (!addr) {
    [city, abbr] = CITY_SUGGEST[match.slug] ?? ["", ""];
    source = "not on live site: suggested, confirm with PMG";
  }
  return {
...
partners.sort((a, b) => a.state.localeCompare(b.state) || a.name.localeCompare(b.name, "en", { sensitivity: "base" }));
...
const education = [];
let heading = "";
for (const m of eduHub.matchAll(/<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>|<a\b[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi)) {
  if (m[1] !== undefined) {
    heading = text(m[1]);
    continue;
  }
  ...
  const canonical = hop?.status === 301 ? new URL(hop.redirect_to).pathname : path;
  ...
  education.push({ category, path: canonical, hub_link: path, hub_title: text(m[3]), note: path !== canonical ? `hub links ${path}, which 301s here` : "" });
}
for (const p of live.filter((p) => /^\/pain-education\/[^/]+\/$/.test(p.path))) {
  if (!education.some((e) => e.path === p.path))
    education.push({ category: "Procedures", path: p.path, hub_link: "", hub_title: "", note: "live and in sitemap, but not linked from the hub; category assigned by topic" });
}
for (const e of education) {
  ...
  e.url = `${ORIGIN}${e.path}`;
  e.viewmedica = [...new Set(page.match(/ondemand\.viewmedica\.com\/\d+\/open\/[\w]+/g) ?? [])].join(" ");
  e.status = p.status;
}
const catOrder = ["Conditions", "Procedures", "Medications"];
education.sort((a, b) => catOrder.indexOf(a.category) - catOrder.indexOf(b.category));
...
for (const p of live.filter((p) => p.path.startsWith("/em_testimonial/"))) {
  ...
  const quote = bodyText(await html(p.path)).replace(/^Home /, "").replace(name, "").trim();
  ...
  testimonials.push({ name, title, organization: org.join(", "), quote, legacy_url: p.path });
}
...
for (const p of live) {
  ...
  const [, month, day, year] = page.match(new RegExp(`(${MONTHS.join("|")}) (\\d{1,2}), (\\d{4})`));
  const date = `${year}-${String(MONTHS.indexOf(month) + 1).padStart(2, "0")}-${day.padStart(2, "0")}`;
  news.push({ date, title: p.title.replace(/ – Pain Management Group$/, ""), path: p.path, proposed: `/news${p.path}` });
}
news.sort((a, b) => a.date.localeCompare(b.date));

// ------------------------------------------------------------ URL plan

const KEEP = new Set(["/", "/our-partners/", "/about-us/", "/contact/", "/pain-education/"]);
const RULES = [
  [/^\/why-choose-us\/$/, "/partnership/how-it-works/", "Rev 2.0"],
  [/^\/(service|provider-opportunities)\/$/, "/providers/", "Rev 2.0"],
  [/^\/internal-team-opportunities\/$/, "/about-us/careers/", "Rev 2.0 page for non-clinical roles; live page is lorem ipsum"],
  [/^\/(blog|blog-left-sidebar)\/$/, "/news/", "news hub; /blog-left-sidebar/ is the live nav's Blog link"],
  [/^\/category\/[^/]+\/$/, "/news/", "blog category archive, linked from every post sidebar"],
  [/^\/author\/[^/]+\/$/, "/news/", "blog author archive, in sitemap"],
  [/^\/\d{4}\/\d{2}\/(\d{2}\/)?$/, "/news/", "blog date archive, linked from every post sidebar"],
  [/^\/(our-clients|em_testimonial\/[^/]+)\/$/, "/results/testimonials/", "carries the 3 named testimonials"],
];
const DEMO = /^\/(blog-(left|right)-2column|blog-right-sidebar|home-one-page|home-video-page-2|portfolio(-3column|-full-3column)?|pricing-plan|sample-page|type\/gallery|slider\/[^/]+|em_team\/[^/]+)\/(page\/\d+\/)?$/;

const plan = [];
for (const p of pages) {
  const row = { url: p.url, path: p.path, title: p.title, status: p.status, indexable: p.indexable, in_sitemap: p.in_sitemap, proposed_destination: "", action: "", reason: "" };
  ...
  if (p.status === 301) {
    ...
    Object.assign(row, { proposed_destination: to, action: trailing ? "handled by trailingSlash" : "301", reason: trailing ? "" : "existing live redirect; preserve" });
  } else if (KEEP.has(p.path) || education.some((e) => e.path === p.path)) {
    Object.assign(row, { proposed_destination: p.path, action: "keep" });
  } else if (post) {
    Object.assign(row, { proposed_destination: post.proposed, action: "301", reason: `news post ${post.date}` });
  } else if (partnerBySlug.has(p.path)) {
    const partner = partnerBySlug.get(p.path);
    Object.assign(row, { proposed_destination: `/our-partners/${partner.state_slug}/`, action: "301", reason: `partner page for ${partner.name}` });
  } else if (rule) {
    Object.assign(row, { proposed_destination: rule[1], action: "301", reason: rule[2] });
  } else if (DEMO.test(p.path)) {
    Object.assign(row, { action: "retire", reason: "theme demo content; no internal links from real pages" });
  } else {
...
if (unclassified.length) throw new Error(`Unclassified URLs:\n${unclassified.map((r) => r.path).join("\n")}`);

const indexable = plan.filter((r) => r.status === 200 && r.indexable === "yes");
await write("urls.csv", indexable, ["url", "title", "status", "proposed_destination", "action"]);
await write("urls-plan.csv", plan, ["url", "title", "status", "indexable", "in_sitemap", "proposed_destination", "action", "reason"]);
await write("redirects.csv", plan.filter((r) => r.action === "301").map((r) => ({ from: r.path, to: r.proposed_destination, reason: r.reason })));
await write("retire.csv", plan.filter((r) => r.action === "retire").map((r) => ({ path: r.path, title: r.title, indexable: r.indexable, in_sitemap: r.in_sitemap })));
await write("education.csv", education, ["category", "title", "slug", "url", "status", "viewmedica", "hub_title", "note"]);
await write("partners.csv", partners);
await write("testimonials.csv", testimonials);
await write("news.csv", news);

const count = (rows, k) => rows.reduce((a, r) => ((a[r[k]] = (a[r[k]] ?? 0) + 1), a), {});
...
console.log("partners:", partners.length, "in", new Set(partners.map((p) => p.state)).size, "states; hub filters:", filters.length, count(partners, "state"));
