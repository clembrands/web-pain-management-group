...
const SKIP_PATH =
  /^\/(wp-admin|wp-json|wp-includes|xmlrpc\.php|cdn-cgi)(\/|$)|^\/wp-login\.php/;
...
function normalize(href, base) {
  ...
  const keep = [...u.searchParams.keys()].filter(
    (k) => k === "p" || k === "page_id",
  );
  u.search = keep.length
    ? `?${keep.map((k) => `${k}=${u.searchParams.get(k)}`).join("&")}`
    : "";
...
async function readSitemaps() {
  ...
  while (queue.length) {
    ...
    if (res.status !== 200 || !/<(urlset|sitemapindex)/.test(res.body))
      continue;
  ...
  return {
    robotsTxt: robots.status === 200 ? robots.body : null,
    sitemaps: [...seen],
    pages,
  };
}
...
function proposed(path, row) {
  ...
  if (path.startsWith("/blog/"))
    return [path.replace(/^\/blog\//, "/news/"), "301"];
...
async function main() {
  ...
  console.log(
    `  sitemaps read: ${sitemaps.length}, URLs listed: ${fromSitemap.size}`,
  );
  ...
  async function visit(url) {
    ...
    for (const tag of html.match(/<(link|meta)\b[^>]*>/gi) ?? []) {
      if (
        /^<link/i.test(tag) &&
        attr(tag, "rel")?.toLowerCase() === "canonical"
      )
        row.canonical = attr(tag, "href") ?? "";
      if (
        /^<meta/i.test(tag) &&
        /^(robots|googlebot)$/i.test(attr(tag, "name") ?? "")
      )
        row.meta_robots = [row.meta_robots, attr(tag, "content")]
          .filter(Boolean)
          .join("; ");
    }
    const canonicalOk = !row.canonical || normalize(row.canonical, url) === url;
    row.indexable =
      !/noindex/i.test(row.meta_robots) && canonicalOk ? "yes" : "no";
    if (/lorem ipsum/i.test(html))
      row.demo_suspect = [row.demo_suspect, "lorem"].filter(Boolean).join("+");
    ...
    const embeds = [
      ...(html.match(/<(iframe|script)\b[^>]*\bsrc\s*=\s*["'][^"']+["']/gi) ??
        []),
    ].map((t) => attr(t, "src"));
    for (const [name, re] of Object.entries(INTEGRATIONS)) {
      if (!re.test(html)) continue;
      const evidence =
        embeds.find((s) => re.test(s)) ?? html.match(re)?.[0] ?? "";
    ...
    for (const m of html.matchAll(
      ...
      if (!n) continue;
      row.outlinks.push(n);
      if (!found.has(n) && found.size < MAX_PAGES) {
        found.set(n, url);
        queue.push(n);
      }
    }
  ...
  const workers = Array.from({ length: CONCURRENCY }, async () => {
    while (i < queue.length) {
      ...
      await visit(url);
      if (rows.size % 25 === 0)
        console.log(`  crawled ${rows.size} / ${queue.length}`);
    }
  });
  ...
  for (const r of all) {
    [r.proposed_destination, r.action] =
      r.status === 200
        ? proposed(r.path, r)
        : ["", r.status >= 300 && r.status < 400 ? "already-redirects" : "n/a"];
  }
  ...
  await writeFile(
    join(OUT, "integrations.csv"),
    toCsv(integrations, ["url", "integration", "evidence"]),
  );
  await writeFile(
    join(OUT, "education.csv"),
    toCsv(await educationBuckets(), ["category", "title", "slug", "url"]),
  );
  await writeFile(
    join(OUT, "raw.json"),
    JSON.stringify(
  ...
  const by = (k) =>
    Object.entries(
      all.reduce((a, r) => ((a[r[k]] = (a[r[k]] ?? 0) + 1), a), {}),
    );
  console.log(
...
async function educationBuckets() {
  ...
  for (const m of res.body.matchAll(
    ...
    const slug =
      n && new URL(n).pathname.match(/^\/pain-education\/([^/]+)\/$/)?.[1];
